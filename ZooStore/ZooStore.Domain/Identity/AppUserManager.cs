    ...
    public class AppUserManager : UserManager<AppUser>
    {
        ...
        public override Task<IdentityResult> CreateAsync(AppUser user, string password)
        {
            try
            {
                ...
                {
                    return Task.FromResult(IdentityResult.Failed("Ошибка"));
                }
                AppUser existUser = _store.Users.Where(u => u.Email == user.Email).FirstOrDefault();
                if (existUser != null)
                {
                    return Task.FromResult(IdentityResult.Failed("Пользователь с электронной почтой["+ user.Email + "] уже существует!"));
                }
            ...
            {
                return Task.FromResult(IdentityResult.Failed("Ошибка БД"));
            }
